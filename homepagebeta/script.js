document.addEventListener('DOMContentLoaded',function(){
  ...
  function setOsThemeAttr(){root.setAttribute('data-os-theme', media.matches ? 'dark' : 'light')}
  ...
  applyTheme()
  media.addEventListener('change',()=>{if(localStorage.getItem('theme')===null){applyTheme()}else{setOsThemeAttr()}})
  ...
  function visibleButtons(){
    if(!grid)return []
    return Array.from(grid.querySelectorAll('.link-button')).filter(el=>el.offsetParent!==null)
  }
  ...
  function finishDiscordAnimation(){
    ...
    setTimeout(()=>{if(text)text.style.display='none';if(icon)icon.style.display='none';if(checkIcon)checkIcon.classList.add('show')},800)
  ...
  const MESSAGE_VERSION='2025-07-07-v2'
  const updateIndicator=document.getElementById('updateIndicator')
  const seenVersion=localStorage.getItem('seenMessageVersion')
  if(updateIndicator && seenVersion!==MESSAGE_VERSION)updateIndicator.classList.add('show')
  window.addEventListener('beforeunload',()=>{localStorage.setItem('seenMessageVersion',MESSAGE_VERSION)})
})
